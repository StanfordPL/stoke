...
processors, there won't be support for AVX2 instructions.  It should run on
newer architectures without problem, but we haven't tested it.
...
folder, perform a 'git pull' and 'make'.  'make test' is optional, but the
...
are tagged by travis CI's internal build ID.  For these, you will still need to
