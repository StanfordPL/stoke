  .text
  .globl strcpy
  .type strcpy, @function

#! file-offset 0x1667e0
#! rip-offset  0x1267e0
#! capacity    192 bytes

# Text                            #  Line  RIP       Bytes  Opcode              
.strcpy:                          #        0x1267e0  0      OPC=<label>         
  nop                             #  1     0x1267e0  1      OPC=nop             
  nop                             #  2     0x1267e1  1      OPC=nop             
  movl %edi, %eax                 #  3     0x1267e2  2      OPC=movl_r32_r32_1  
  movl %esi, %edi                 #  4     0x1267e4  2      OPC=movl_r32_r32    
  nop                             #  5     0x1267e6  1      OPC=nop             
  nop                             #  6     0x1267e7  1      OPC=nop             
  nop                             #  7     0x1267e8  1      OPC=nop             
  movq %rdi, %rdx                 #  8     0x1267e9  3      OPC=movq_r64_r64    
  andb $0x1, %dil                 #  9     0x1267ec  4      OPC=andb_r8_imm8    
  nop                             #  10    0x1267f0  1      OPC=nop             
  nop                             #  11    0x1267f1  1      OPC=nop             
  nop                             #  12    0x1267f2  1      OPC=nop             
  movl %eax, %ecx                 #  13    0x1267f3  2      OPC=movl_r32_r32_1  
  jne .L_126860                   #  14    0x1267f5  6      OPC=jne_label_1     
  nop                             #  15    0x1267fb  1      OPC=nop             
  nop                             #  16    0x1267fc  1      OPC=nop             
  nop                             #  17    0x1267fd  1      OPC=nop             
  nop                             #  18    0x1267fe  1      OPC=nop             
  nop                             #  19    0x1267ff  1      OPC=nop             
  nop                             #  20    0x126800  1      OPC=nop             
  nop                             #  21    0x126801  1      OPC=nop             
  nop                             #  22    0x126802  1      OPC=nop             
  xorl %ebp, %esi                 #  23    0x126803  2      OPC=xorl_r32_r32_1  
  movl (%r15,%rsi,1), %r8d        #  24    0x126805  4      OPC=movl_r32_m32    
  movl %r8d, %esi                 #  25    0x126809  3      OPC=movl_r32_r32    
  nop                             #  26    0x12680c  1      OPC=nop             
  nop                             #  27    0x12680d  1      OPC=nop             
  andl $0x80808080, %edi          #  28    0x12680e  6      OPC=andl_r32_imm32  
  notl %esi                       #  29    0x126814  2      OPC=notl_r32        
  nop                             #  30    0x126816  1      OPC=nop             
  nop                             #  31    0x126817  1      OPC=nop             
  andl %edi, %esi                 #  32    0x126818  2      OPC=andl_r32_r32    
  jne .L_126860                   #  33    0x12681a  6      OPC=jne_label_1     
.L_126820:                        #        0x126820  0      OPC=<label>         
  movq %rbp, %r9                  #  34    0x126820  3      OPC=movq_r64_r64    
  addl $0x4, %ecx                 #  35    0x126823  3      OPC=addl_r32_imm8   
  movq %r8, -0x4(%r15,%rcx,1)     #  36    0x126826  5      OPC=movq_m64_r64    
  addl $0x4, %edx                 #  37    0x12682b  6      OPC=addl_r32_imm32  
  movl (%r15,%rdx,1), %r8d        #  38    0x126831  4      OPC=movl_r32_m32    
  leaq -0x1010101(%r8), %rsi      #  39    0x126835  7      OPC=leaq_r64_m16    
  nop                             #  40    0x12683c  1      OPC=nop             
  nop                             #  41    0x12683d  1      OPC=nop             
  nop                             #  42    0x12683e  1      OPC=nop             
  nop                             #  43    0x12683f  1      OPC=nop             
  nop                             #  44    0x126840  1      OPC=nop             
  nop                             #  45    0x126841  1      OPC=nop             
  nop                             #  46    0x126842  1      OPC=nop             
  andq %rsi, %r9                  #  47    0x126843  3      OPC=andq_r64_r64    
  je .L_126820                    #  48    0x126846  6      OPC=je_label_1      
  nop                             #  49    0x12684c  1      OPC=nop             
  nop                             #  50    0x12684d  1      OPC=nop             
  nop                             #  51    0x12684e  1      OPC=nop             
  nop                             #  52    0x12684f  1      OPC=nop             
  nop                             #  53    0x126850  1      OPC=nop             
  nop                             #  54    0x126851  1      OPC=nop             
  nop                             #  55    0x126852  1      OPC=nop             
  nop                             #  56    0x126853  1      OPC=nop             
  nop                             #  57    0x126854  1      OPC=nop             
  nop                             #  58    0x126855  1      OPC=nop             
  nop                             #  59    0x126856  1      OPC=nop             
  nop                             #  60    0x126857  1      OPC=nop             
  nop                             #  61    0x126858  1      OPC=nop             
  nop                             #  62    0x126859  1      OPC=nop             
  nop                             #  63    0x12685a  1      OPC=nop             
  nop                             #  64    0x12685b  1      OPC=nop             
  nop                             #  65    0x12685c  1      OPC=nop             
  nop                             #  66    0x12685d  1      OPC=nop             
  nop                             #  67    0x12685e  1      OPC=nop             
  nop                             #  68    0x12685f  1      OPC=nop             
.L_126860:                        #        0x126860  0      OPC=<label>         
  addl $0x1, %edx                 #  69    0x126860  6      OPC=addl_r32_imm32  
  movzbl -0x1(%r15,%rdx,1), %esi  #  70    0x126866  6      OPC=movzbl_r32_m8   
  addl $0x1, %ecx                 #  71    0x12686c  3      OPC=addl_r32_imm8   
  movb %sil, -0x1(%r15,%rcx,1)    #  72    0x12686f  5      OPC=movb_m8_r8      
  testb %sil, %sil                #  73    0x126874  3      OPC=testb_r8_r8     
  jne .L_126860                   #  74    0x126877  6      OPC=jne_label_1     
  nop                             #  75    0x12687d  1      OPC=nop             
  nop                             #  76    0x12687e  1      OPC=nop             
  nop                             #  77    0x12687f  1      OPC=nop             
  nop                             #  78    0x126880  1      OPC=nop             
  nop                             #  79    0x126881  1      OPC=nop             
  nop                             #  80    0x126882  1      OPC=nop             
  nop                             #  81    0x126883  1      OPC=nop             
  nop                             #  82    0x126884  1      OPC=nop             
  nop                             #  83    0x126885  1      OPC=nop             
  nop                             #  84    0x126886  1      OPC=nop             
  nop                             #  85    0x126887  1      OPC=nop             
  nop                             #  86    0x126888  1      OPC=nop             
  nop                             #  87    0x126889  1      OPC=nop             
  nop                             #  88    0x12688a  1      OPC=nop             
  nop                             #  89    0x12688b  1      OPC=nop             
  retq

.size strcpy, -.strcpy

