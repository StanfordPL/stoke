  .text
  .globl strcpy
  .type strcpy, @function

#! file-offset 0x1667e0
#! rip-offset  0x1267e0
#! capacity    192 bytes

# Text                            #  Line  RIP       Bytes  Opcode              
.strcpy:                          #        0x1267e0  0      OPC=<label>         
  nop                             #  1     0x1267e0  1      OPC=nop             
  nop                             #  2     0x1267e1  1      OPC=nop             
  nop                             #  3     0x1267e2  1      OPC=nop             
  movl %edi, %ecx                 #  4     0x1267e3  2      OPC=movl_r32_r32    
  movl %esi, %edx                 #  5     0x1267e5  2      OPC=movl_r32_r32_1  
  nop                             #  6     0x1267e7  1      OPC=nop             
  nop                             #  7     0x1267e8  1      OPC=nop             
  nop                             #  8     0x1267e9  1      OPC=nop             
  nop                             #  9     0x1267ea  1      OPC=nop             
  movl %edi, %eax                 #  10    0x1267eb  2      OPC=movl_r32_r32_1  
  subb %dl, %dil                  #  11    0x1267ed  3      OPC=subb_r8_r8_1    
  andq $0x3, %rdi                 #  12    0x1267f0  4      OPC=andq_r64_imm8   
  nop                             #  13    0x1267f4  1      OPC=nop             
  nop                             #  14    0x1267f5  1      OPC=nop             
  nop                             #  15    0x1267f6  1      OPC=nop             
  nop                             #  16    0x1267f7  1      OPC=nop             
  nop                             #  17    0x1267f8  1      OPC=nop             
  nop                             #  18    0x1267f9  1      OPC=nop             
  nop                             #  19    0x1267fa  1      OPC=nop             
  nop                             #  20    0x1267fb  1      OPC=nop             
  nop                             #  21    0x1267fc  1      OPC=nop             
  nop                             #  22    0x1267fd  1      OPC=nop             
  nop                             #  23    0x1267fe  1      OPC=nop             
  nop                             #  24    0x1267ff  1      OPC=nop             
  jne .L_126860                   #  25    0x126800  6      OPC=jne_label_1     
  nop                             #  26    0x126806  1      OPC=nop             
  nop                             #  27    0x126807  1      OPC=nop             
  nop                             #  28    0x126808  1      OPC=nop             
  nop                             #  29    0x126809  1      OPC=nop             
  nop                             #  30    0x12680a  1      OPC=nop             
  nop                             #  31    0x12680b  1      OPC=nop             
  nop                             #  32    0x12680c  1      OPC=nop             
  nop                             #  33    0x12680d  1      OPC=nop             
  nop                             #  34    0x12680e  1      OPC=nop             
  nop                             #  35    0x12680f  1      OPC=nop             
  nop                             #  36    0x126810  1      OPC=nop             
  movl %esi, %esi                 #  37    0x126811  2      OPC=movl_r32_r32    
  movl (%r15,%rsi,1), %r8d        #  38    0x126813  4      OPC=movl_r32_m32    
  nop                             #  39    0x126817  1      OPC=nop             
  nop                             #  40    0x126818  1      OPC=nop             
  nop                             #  41    0x126819  1      OPC=nop             
  jne .L_126860                   #  42    0x12681a  6      OPC=jne_label_1     
.L_126820:                        #        0x126820  0      OPC=<label>         
  subl %ebp, %ecx                 #  43    0x126820  2      OPC=subl_r32_r32_1  
  movl %r8d, (%r15,%rcx,1)        #  44    0x126822  4      OPC=movl_m32_r32    
  addl $0x4, %edx                 #  45    0x126826  6      OPC=addl_r32_imm32  
  movq (%r15,%rdx,1), %r8         #  46    0x12682c  4      OPC=movq_r64_m64    
  leaq -0x1010101(%r8), %rsi      #  47    0x126830  7      OPC=leaq_r64_m32    
  movq %r8, %r9                   #  48    0x126837  3      OPC=movq_r64_r64    
  andl $0x80808080, %esi          #  49    0x12683a  6      OPC=andl_r32_imm32  
  addl $0x4, %ecx                 #  50    0x126840  3      OPC=addl_r32_imm8   
  notl %r9d                       #  51    0x126843  3      OPC=notl_r32        
  andl %esi, %r9d                 #  52    0x126846  3      OPC=andl_r32_r32_1  
  je .L_126820                    #  53    0x126849  6      OPC=je_label_1      
  nop                             #  54    0x12684f  1      OPC=nop             
  nop                             #  55    0x126850  1      OPC=nop             
  nop                             #  56    0x126851  1      OPC=nop             
  nop                             #  57    0x126852  1      OPC=nop             
  nop                             #  58    0x126853  1      OPC=nop             
  nop                             #  59    0x126854  1      OPC=nop             
  nop                             #  60    0x126855  1      OPC=nop             
  nop                             #  61    0x126856  1      OPC=nop             
  nop                             #  62    0x126857  1      OPC=nop             
  nop                             #  63    0x126858  1      OPC=nop             
  nop                             #  64    0x126859  1      OPC=nop             
  nop                             #  65    0x12685a  1      OPC=nop             
  nop                             #  66    0x12685b  1      OPC=nop             
  nop                             #  67    0x12685c  1      OPC=nop             
  nop                             #  68    0x12685d  1      OPC=nop             
  nop                             #  69    0x12685e  1      OPC=nop             
  nop                             #  70    0x12685f  1      OPC=nop             
.L_126860:                        #        0x126860  0      OPC=<label>         
  incl %edx                       #  71    0x126860  2      OPC=incl_r32        
  movzbq -0x1(%r15,%rdx,1), %rsi  #  72    0x126862  6      OPC=movzbq_r64_m8   
  addl $0x1, %ecx                 #  73    0x126868  6      OPC=addl_r32_imm32  
  movb %sil, -0x1(%r15,%rcx,1)    #  74    0x12686e  5      OPC=movb_m8_r8      
  testb %sil, %sil                #  75    0x126873  3      OPC=testb_r8_r8     
  jne .L_126860                   #  76    0x126876  6      OPC=jne_label_1     
  nop                             #  77    0x12687c  1      OPC=nop             
  nop                             #  78    0x12687d  1      OPC=nop             
  nop                             #  79    0x12687e  1      OPC=nop             
  nop                             #  80    0x12687f  1      OPC=nop             
  nop                             #  81    0x126880  1      OPC=nop             
  nop                             #  82    0x126881  1      OPC=nop             
  nop                             #  83    0x126882  1      OPC=nop             
  nop                             #  84    0x126883  1      OPC=nop             
  nop                             #  85    0x126884  1      OPC=nop             
  nop                             #  86    0x126885  1      OPC=nop             
  nop                             #  87    0x126886  1      OPC=nop             
  nop                             #  88    0x126887  1      OPC=nop             
  nop                             #  89    0x126888  1      OPC=nop             
  nop                             #  90    0x126889  1      OPC=nop             
  retq

.size strcpy, -.strcpy

