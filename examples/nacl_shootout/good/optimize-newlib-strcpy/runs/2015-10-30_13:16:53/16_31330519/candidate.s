  .text
  .globl strcpy
  .type strcpy, @function

#! file-offset 0x1667e0
#! rip-offset  0x1267e0
#! capacity    192 bytes

# Text                        #  Line  RIP       Bytes  Opcode              
.strcpy:                      #        0x1267e0  0      OPC=<label>         
  movq %rsi, %rdx             #  1     0x1267e0  3      OPC=movq_r64_r64    
  nop                         #  2     0x1267e3  1      OPC=nop             
  nop                         #  3     0x1267e4  1      OPC=nop             
  movl %edi, %eax             #  4     0x1267e5  2      OPC=movl_r32_r32    
  movl %edx, %edi             #  5     0x1267e7  2      OPC=movl_r32_r32    
  nop                         #  6     0x1267e9  1      OPC=nop             
  andl $0x3, %edi             #  7     0x1267ea  3      OPC=andl_r32_imm8   
  movq %rax, %rcx             #  8     0x1267ed  3      OPC=movq_r64_r64    
  nop                         #  9     0x1267f0  1      OPC=nop             
  nop                         #  10    0x1267f1  1      OPC=nop             
  nop                         #  11    0x1267f2  1      OPC=nop             
  jne .L_126860               #  12    0x1267f3  6      OPC=jne_label_1     
  nop                         #  13    0x1267f9  1      OPC=nop             
  movl %esi, %esi             #  14    0x1267fa  2      OPC=movl_r32_r32    
  movl (%r15,%rsi,1), %r8d    #  15    0x1267fc  4      OPC=movl_r32_m32    
  nop                         #  16    0x126800  1      OPC=nop             
  leal -0x1010101(%r8), %edi  #  17    0x126801  7      OPC=leal_r32_m32    
  movl %r8d, %esi             #  18    0x126808  3      OPC=movl_r32_r32    
  notl %esi                   #  19    0x12680b  2      OPC=notl_r32        
  andl $0x80808080, %esi      #  20    0x12680d  6      OPC=andl_r32_imm32  
  andl %edi, %esi             #  21    0x126813  2      OPC=andl_r32_r32    
  jne .L_126860               #  22    0x126815  6      OPC=jne_label_1     
  nop                         #  23    0x12681b  1      OPC=nop             
  nop                         #  24    0x12681c  1      OPC=nop             
  nop                         #  25    0x12681d  1      OPC=nop             
  nop                         #  26    0x12681e  1      OPC=nop             
  nop                         #  27    0x12681f  1      OPC=nop             
.L_126820:                    #        0x126820  0      OPC=<label>         
  addw $0x4, %dx              #  28    0x126820  4      OPC=addw_r16_imm8   
  movl %ecx, %ecx             #  29    0x126824  2      OPC=movl_r32_r32_1  
  movl %r8d, (%r15,%rcx,1)    #  30    0x126826  4      OPC=movl_m32_r32    
  addl $0x4, %ecx             #  31    0x12682a  3      OPC=addl_r32_imm8   
  andl %edx, %edx             #  32    0x12682d  2      OPC=andl_r32_r32_1  
  movl (%r15,%rdx,1), %r8d    #  33    0x12682f  4      OPC=movl_r32_m32    
  movq %r8, %r9               #  34    0x126833  3      OPC=movq_r64_r64_1  
  notl %r9d                   #  35    0x126836  3      OPC=notl_r32        
  leal -0x1010101(%r8), %esi  #  36    0x126839  7      OPC=leal_r32_m16    
  andl %esi, %r9d             #  37    0x126840  3      OPC=andl_r32_r32    
  andl $0x80808080, %r9d      #  38    0x126843  7      OPC=andl_r32_imm32  
  je .L_126820                #  39    0x12684a  6      OPC=je_label_1      
  nop                         #  40    0x126850  1      OPC=nop             
  nop                         #  41    0x126851  1      OPC=nop             
  nop                         #  42    0x126852  1      OPC=nop             
  nop                         #  43    0x126853  1      OPC=nop             
  nop                         #  44    0x126854  1      OPC=nop             
  nop                         #  45    0x126855  1      OPC=nop             
  nop                         #  46    0x126856  1      OPC=nop             
  nop                         #  47    0x126857  1      OPC=nop             
  nop                         #  48    0x126858  1      OPC=nop             
  nop                         #  49    0x126859  1      OPC=nop             
  nop                         #  50    0x12685a  1      OPC=nop             
  nop                         #  51    0x12685b  1      OPC=nop             
  nop                         #  52    0x12685c  1      OPC=nop             
  nop                         #  53    0x12685d  1      OPC=nop             
  nop                         #  54    0x12685e  1      OPC=nop             
  nop                         #  55    0x12685f  1      OPC=nop             
.L_126860:                    #        0x126860  0      OPC=<label>         
  movl %edx, %edx             #  56    0x126860  2      OPC=movl_r32_r32    
  movzbq (%r15,%rdx,1), %rsi  #  57    0x126862  5      OPC=movzbq_r64_m8   
  addl $0x1, %edx             #  58    0x126867  3      OPC=addl_r32_imm8   
  orl %ecx, %ecx              #  59    0x12686a  2      OPC=orl_r32_r32_1   
  movb %sil, (%r15,%rcx,1)    #  60    0x12686c  4      OPC=movb_m8_r8      
  addl $0x1, %ecx             #  61    0x126870  3      OPC=addl_r32_imm8   
  testb %sil, %sil            #  62    0x126873  3      OPC=testb_r8_r8     
  jne .L_126860               #  63    0x126876  6      OPC=jne_label_1     
  nop                         #  64    0x12687c  1      OPC=nop             
  nop                         #  65    0x12687d  1      OPC=nop             
  nop                         #  66    0x12687e  1      OPC=nop             
  nop                         #  67    0x12687f  1      OPC=nop             
  retq

.size strcpy, -.strcpy

