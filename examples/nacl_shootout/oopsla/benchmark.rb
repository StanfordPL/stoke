...
class BenchmarkJob < Job
  ...
  def run
    ...
    if @id == "result" then
      FileUtils.cp("#{mk_folder(@benchmark, @mode, "")}/result.s", 
                   "#{mk_folder(@benchmark, @mode, "search")}/outputs")
    end
...
def main
  ...
  all.each do |bench|
    modes.each do |mode| 
      ids = Dir.glob("#{mk_folder(bench, mode, "benchmark")}/*.out").map do |outfile|
      ...
      ids = ["0", "result"] if $global_settings[:result_only]
