  .text
  .globl wmemcmp
  .type wmemcmp, @function

#! file-offset 0x18f2a0
#! rip-offset  0x14f2a0
#! capacity    160 bytes

# Text                          #  Line  RIP       Bytes  Opcode              
.wmemcmp:                       #        0x14f2a0  0      OPC=<label>         
  nop                           #  1     0x14f2a0  1      OPC=nop             
  testl %edx, %edx              #  2     0x14f2a1  2      OPC=testl_r32_r32   
  je .L_14f300                  #  3     0x14f2a3  2      OPC=je_label        
  nop                           #  4     0x14f2a5  1      OPC=nop             
  xorl %eax, %eax               #  5     0x14f2a6  2      OPC=xorl_r32_r32    
  nop                           #  6     0x14f2a8  1      OPC=nop             
  nop                           #  7     0x14f2a9  1      OPC=nop             
  nop                           #  8     0x14f2aa  1      OPC=nop             
  nop                           #  9     0x14f2ab  1      OPC=nop             
  nop                           #  10    0x14f2ac  1      OPC=nop             
  nop                           #  11    0x14f2ad  1      OPC=nop             
  nop                           #  12    0x14f2ae  1      OPC=nop             
  andl %edi, %edi               #  13    0x14f2af  2      OPC=andl_r32_r32_1  
  movl (%r15,%rdi,1), %r8d      #  14    0x14f2b1  4      OPC=movl_r32_m32    
  nop                           #  15    0x14f2b5  1      OPC=nop             
  orl %esi, %esi                #  16    0x14f2b6  2      OPC=orl_r32_r32     
  movl (%r15,%rsi,1), %ecx      #  17    0x14f2b8  4      OPC=movl_r32_m32    
  nop                           #  18    0x14f2bc  1      OPC=nop             
  nop                           #  19    0x14f2bd  1      OPC=nop             
  nop                           #  20    0x14f2be  1      OPC=nop             
  nop                           #  21    0x14f2bf  1      OPC=nop             
  nop                           #  22    0x14f2c0  1      OPC=nop             
  nop                           #  23    0x14f2c1  1      OPC=nop             
  nop                           #  24    0x14f2c2  1      OPC=nop             
  nop                           #  25    0x14f2c3  1      OPC=nop             
  nop                           #  26    0x14f2c4  1      OPC=nop             
  cmpl %ecx, %r8d               #  27    0x14f2c5  3      OPC=cmpl_r32_r32    
  je .L_14f2e0                  #  28    0x14f2c8  2      OPC=je_label        
  jmpq .L_14f320                #  29    0x14f2ca  2      OPC=jmpq_label      
.L_14f2c0:                      #        0x14f2cc  0      OPC=<label>         
  nop                           #  30    0x14f2cc  1      OPC=nop             
  nop                           #  31    0x14f2cd  1      OPC=nop             
  nop                           #  32    0x14f2ce  1      OPC=nop             
  addl $0x4, %edi               #  33    0x14f2cf  3      OPC=addl_r32_imm8   
  movl (%r15,%rdi,1), %r8d      #  34    0x14f2d2  4      OPC=movl_r32_m32    
  addl $0x4, %esi               #  35    0x14f2d6  3      OPC=addl_r32_imm8   
  movl (%r15,%rsi,1), %ecx      #  36    0x14f2d9  4      OPC=movl_r32_m32    
  cmpl %ecx, %r8d               #  37    0x14f2dd  3      OPC=cmpl_r32_r32    
  jne .L_14f320                 #  38    0x14f2e0  2      OPC=jne_label       
  nop                           #  39    0x14f2e2  1      OPC=nop             
  nop                           #  40    0x14f2e3  1      OPC=nop             
  nop                           #  41    0x14f2e4  1      OPC=nop             
  nop                           #  42    0x14f2e5  1      OPC=nop             
  nop                           #  43    0x14f2e6  1      OPC=nop             
  nop                           #  44    0x14f2e7  1      OPC=nop             
  nop                           #  45    0x14f2e8  1      OPC=nop             
.L_14f2e0:                      #        0x14f2e9  0      OPC=<label>         
  addl $0x1, %eax               #  46    0x14f2e9  5      OPC=addl_eax_imm32  
  cmpl %eax, %edx               #  47    0x14f2ee  2      OPC=cmpl_r32_r32    
  ja .L_14f2c0                  #  48    0x14f2f0  2      OPC=ja_label        
  nop                           #  49    0x14f2f2  1      OPC=nop             
  nop                           #  50    0x14f2f3  1      OPC=nop             
  nop                           #  51    0x14f2f4  1      OPC=nop             
  nop                           #  52    0x14f2f5  1      OPC=nop             
  nop                           #  53    0x14f2f6  1      OPC=nop             
.L_14f300:                      #        0x14f2f7  0      OPC=<label>         
  nop                           #  54    0x14f2f7  1      OPC=nop             
  nop                           #  55    0x14f2f8  1      OPC=nop             
  nop                           #  56    0x14f2f9  1      OPC=nop             
  xorl %eax, %eax               #  57    0x14f2fa  2      OPC=xorl_r32_r32    
  nop                           #  58    0x14f2fc  1      OPC=nop             
  nop                           #  59    0x14f2fd  1      OPC=nop             
  nop                           #  60    0x14f2fe  1      OPC=nop             
  nop                           #  61    0x14f2ff  1      OPC=nop             
  nop                           #  62    0x14f300  1      OPC=nop             
  nop                           #  63    0x14f301  1      OPC=nop             
  nop                           #  64    0x14f302  1      OPC=nop             
  nop                           #  65    0x14f303  1      OPC=nop             
  nop                           #  66    0x14f304  1      OPC=nop             
  retq                          #  67    0x14f305  1      OPC=retq            
  nop                           #  68    0x14f306  1      OPC=nop             
  nop                           #  69    0x14f307  1      OPC=nop             
  nop                           #  70    0x14f308  1      OPC=nop             
  nop                           #  71    0x14f309  1      OPC=nop             
  nop                           #  72    0x14f30a  1      OPC=nop             
  nop                           #  73    0x14f30b  1      OPC=nop             
  nop                           #  74    0x14f30c  1      OPC=nop             
  nop                           #  75    0x14f30d  1      OPC=nop             
  nop                           #  76    0x14f30e  1      OPC=nop             
.L_14f320:                      #        0x14f30f  0      OPC=<label>         
  nop                           #  77    0x14f30f  1      OPC=nop             
  nop                           #  78    0x14f310  1      OPC=nop             
  nop                           #  79    0x14f311  1      OPC=nop             
  nop                           #  80    0x14f312  1      OPC=nop             
  nop                           #  81    0x14f313  1      OPC=nop             
  nop                           #  82    0x14f314  1      OPC=nop             
  subl %eax, %eax               #  83    0x14f315  2      OPC=subl_r32_r32_1  
  nop                           #  84    0x14f317  1      OPC=nop             
  nop                           #  85    0x14f318  1      OPC=nop             
  nop                           #  86    0x14f319  1      OPC=nop             
  cmpl %ecx, %r8d               #  87    0x14f31a  3      OPC=cmpl_r32_r32    
  nop                           #  88    0x14f31d  1      OPC=nop             
  nop                           #  89    0x14f31e  1      OPC=nop             
  nop                           #  90    0x14f31f  1      OPC=nop             
  nop                           #  91    0x14f320  1      OPC=nop             
  setg %al                      #  92    0x14f321  3      OPC=setg_r8         
  nop                           #  93    0x14f324  1      OPC=nop             
  leal -0x1(%rax,%rax,1), %eax  #  94    0x14f325  4      OPC=leal_r32_m32    
  retq                          #  95    0x14f329  1      OPC=retq            
                                                                              
.size wmemcmp, .-wmemcmp
