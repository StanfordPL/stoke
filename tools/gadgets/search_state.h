...
public:
  SearchStateGadget(const Cfg& target, const std::vector<TUnit>& aux_fxns) {
    current = CfgGadget(current_arg.value().get_code(), aux_fxns);
    best_yet = CfgGadget(best_yet_arg.value().get_code(), aux_fxns);
    best_correct = CfgGadget(best_correct_arg.value().get_code(), aux_fxns);
