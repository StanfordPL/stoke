...
auto& index = ValueArg<size_t>::create("index")
  .usage("<int>")
...
void callback(size_t line, const CpuState* cs, void* arg) { 
  ...
  cout << "Error Code: " << (int)cs->ec << endl;
  cout << endl;
	cout << *cs << endl; 
	cout << endl;
...
int main(int argc, char** argv) {
  ...
  DebugHandler::install();
    ...
    .set_read_only_mem(false)
		.insert_input(testcases.value()[min(testcases.value().size()-1, index.value())]);
	...
	if ( result.ec != ErrorCode::NORMAL ) {
		cout << "Function exited abnormally with error code: ";
    cout << (int)result.ec << endl;
	} else {
